...
Display filtered results in a [Table]({% link _components/table.md %}) or a card list below the filters.
...
**[Filter sidebar]({% link _components/filter-sidebar.md %})** — use when filters need to be quickly accessed on a regular basis and should always be visible alongside the listing. The filter sidebar places filters in a persistent left-hand column. Because filters are always visible, applied filter tags are not required.
...
Always include an [empty state]({% link _patterns/empty-state.md %}) for when filters return no results.

## Related components

- [Filter sidebar]({% link _components/filter-sidebar.md %}) — the persistent sidebar component for always-visible filters.
- [Table]({% link _components/table.md %}) — filterable and sortable table for displaying results.

## Related patterns

- [Empty state]({% link _patterns/empty-state.md %}) — what to show when filters return no results.
- [Pagination]({% link _patterns/pagination.md %}) — use alongside filters on large datasets.
