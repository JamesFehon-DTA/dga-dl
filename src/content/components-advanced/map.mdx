...
Use a Map to embed an interactive map showing a physical location – an office, a service centre, a venue. The component pairs the embedded map with a plain-text address and a link that opens the location in the map provider's own site, so the map is never the only way to find the place. Editors set the embed URL, the address text, and the external link in the CMS.
...
- users need to search, filter, or compare many locations – that is an application, not an embed
