...
- **Reference pages (glossaries, checklists, communiques):** zero. Reference content is read by lookup; callouts pull the reader's eye away from the lookup target.
- **Reference FAQ:** zero or one – use one above the accordion only when 80%+ of readers need the same answer.
- **Section landing pages:** zero. Routing pages do not need callouts.
